...
function main(): void {
  ...
  expectEqual('20 buku -> 1 halaman (kapasitas hal1 == 20)', paginateBorrowCard(20).length, 1)
  const p21 = paginateBorrowCard(21)
  expectEqual('21 buku -> 2 halaman (20+1)', p21.length, 2)
  ...
  expectEqual('hal2 memuat 27 (kapasitas lanjutan >= 27)', p47[1].endIndex - p47[1].startIndex, 27)
  const p48 = paginateBorrowCard(48)
  expectEqual('48 buku -> 3 halaman (20+27+1)', p48.length, 3)
  expectEqual('hal1 memuat 20', p48[0].endIndex - p48[0].startIndex, 20)
  expectEqual('hal2 memuat 27', p48[1].endIndex - p48[1].startIndex, 27)
  ...
  check('line-height baris 4.4mm', html1.includes('line-height: 4.4mm;'))
  ...
  check('body 26mm + pemisah border-bottom abu terang', html1.includes('.body { display: flex; gap: 3mm; height: 26mm; margin-top: 0; margin-bottom: 0; align-items: stretch; border-bottom: 1px solid #e2e8f0; }'))
  check('avatar menyesuaikan body 26mm', html1.includes('.avatar { width: 20mm; height: 20mm; flex: 0 0 20mm;'))
  check('footer 12mm', html1.includes('.footer { display: flex; align-items: flex-end; gap: 4mm; height: 12mm;'))
  check('QR rata kanan (margin-left auto)', html1.includes('.qr { width: 12mm; height: 12mm; flex: 0 0 12mm; margin-left: auto; }'))
  ...
  check('hal1 memuat buku 1..20', pages[0].includes('Buku Ke-1') && pages[0].includes('Buku Ke-20') && !pages[0].includes('Buku Ke-21'))
  check('hal2 memuat buku 21..47', pages[1].includes('Buku Ke-21') && pages[1].includes('Buku Ke-47') && !pages[1].includes('Buku Ke-48'))
  check('hal3 memuat buku 48', pages[2].includes('Buku Ke-48'))
  const manyHtml = generateBorrowCardHtml(many)
  check('tiap kartu punya header-info (Jumlah: 48)', countOccurrences(manyHtml, 'class="header-info"') === 3)
