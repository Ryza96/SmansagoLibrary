...
- **Kontrak data (R-1, dikonfirmasi):** **Status Keanggotaan AKTIF = pernah memiliki `MemberEnrollment`** (enrollment terminal DROPPED tetap AKTIF); m3 `status=ACTIVE` tanpa enrollment + PINJAMAN AKTIF → NONAKTIF (bukan dari pinjaman); **Kelas = SSOT `MemberEnrollment` ACTIVE** (terminal → null); **Tanggal Bergabung = `Member.createdAt`**; **ringkasan mengikuti filter** (search + status + kelas) — `summary.total == active + nonActive == pagination.total`; pagination murni view (summary stabil antar-halaman); **kombinasi NONAKTIF + Kelas → 0** (some+none di-AND Prisma — anggota berkelas pasti pernah enrollment).
