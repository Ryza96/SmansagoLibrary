...
## WO-3 (Provider Framework): BackupProvider/RestoreHandler contracts + registries + preflight + DatabaseProvider (COMPLETE - READY review PO, ter-release)

### Ringkasan
- Source of Truth: `ADR_001_DATA_PROTECTION_FINAL_DECISIONS.md` (FINAL APPROVED, SSOT) + `RFC_003_BACKUP_ENGINE_ARCHITECTURE.md` + `RFC_004_RESTORE_ENGINE_ARCHITECTURE.md`. Scope: **fondasi Provider** - kontrak `BackupProvider` (id/version - required/optional - collect/verify/cleanup), kontrak `RestoreHandler` (matches/stage/verifyStaged/swapToLive/rollbackFrom/cleanup - swapToLive HANYA dipanggil Swapper), dua Registry, model Pre-flight & Failure Decision (`decideProviderPreflight`), serta `DatabaseProvider` (satu-satunya jenis data rilis pertama: Database WAJIB, ADR-001 §8.2). **BUKAN** Backup/Restore Engine, ZIP, Manifest Builder, UI, IPC.
- **File baru (8):** `src/main/domain/provider/domain-error.ts` (`ProviderDomainError`), `provider-id.ts` (`ProviderId` VO name+version ≤64/≤32, `fullName = name@version`, equals, toJSON), `provider-kind.ts` (`PROVIDER_KINDS` database/asset/configuration/log), `provider.ts` (kontrak + `PROVIDER_REQUIREMENTS` + `collectResultOf`/`verifyResultOf`), `restore-handler.ts` (kontrak + `restoreVerifyResultOf` + `assertProviderRequirement`), `provider-registry.ts` (`ProviderRegistry` + `RestoreHandlerRegistry` - anti-duplikat via equals), `preflight.ts` (`PreflightItem`/`PreflightReport` + `decideProviderPreflight` murni), `src/main/infrastructure/providers/database.provider.ts` (`DatabaseProvider` - `VACUUM INTO` via `getPrisma()` + stat + `PRAGMA integrity_check`; `DATABASE_SNAPSHOT_FILENAME='aplibrary.db'`, engine `vacuum-into`).
- **Dimodifikasi (1):** `electron/main/bootstrap.ts` - Container +`databaseProvider` + `providerRegistry` + `restoreHandlerRegistry` (kosong). **TIDAK diubah:** schema/migration, IPC/preload/env.d.ts/renderer/UI.
- **Validation PASS:** lint; build (main **1,893,255 B** - preload 9.95 kB identik - renderer 1,147.66 kB identik); smoke `wo3_provider_smoke` **111/111** (ProviderId 15, Kind & Requirement 9, contract 12, restore contract 9, ProviderRegistry 15, RestoreHandlerRegistry 9, preflight 24, DatabaseProvider DB-backed 18); `prisma migrate diff` empty; grep bundle `providerRegistry`/`RestoreHandlerRegistry`/`aplibrary.db`/`VACUUM INTO` = ter-wire.
- **Laporan:** `WORK_ORDER_3_PROVIDER_FRAMEWORK.md`. Status: **DONE - READY review PO**.
- **Commit:** `2a4af89` (+ laporan; AGENTS.md update menyusul di WO-5).

### Pelajaran (retain)
- **Provider & Restore Handler = dua sisi dari SATU jenis data** (K2) - keduanya didaftarkan via registry masing-masing; engine memanggil via kontrak seragam.
- **Kontrak `collect()` = metadata awal `{kind, relativePath, sizeBytes}`** - checksum dihitung Manifest Builder (RFC-003 §5), bukan provider; `verify(entry)` menerima `ManifestEntry` (kontrak pasif dibagikan lintas arah).
- **`swapToLive` HANYA dipanggil Swapper** - invarian ADR-001 §3.3/§7 (Swapper = satu-satunya penulis DB live/aset) ditegaskan di kontrak, dijalankan Restore Engine WO-5.
- **Failure Strategy = fungsi murni `decideProviderPreflight`** - FAIL required → abort, FAIL optional → skip+warning (SUCCESS_WITH_WARNING), warning → warning; K5/K7 (batalkan SEBELUM menulis) diekspresikan headless-testable.
- **VACUUM INTO = metode snapshot konsisten** (ADR-001 §3.2) - `$executeRawUnsafe` via `getPrisma()`; unlink target dulu (edge `output file already exists`); verifikasi via PrismaClient terpisah point-ke-snapshot.

---

## WO-4 (Backup Engine): BackupService orchestrator + SchemaVersionReader + ManifestBuilder + Packager + Verifier, wired ke Container (COMPLETE - READY review PO, ter-release)

### Ringkasan
- Source of Truth: ADR-001 (FINAL APPROVED, SSOT) + RFC-002/003/004. Scope: **Backup Engine lengkap** - orkestrator `BackupService` + 4 komponen infra + wiring Container. **TANPA** UI, IPC, Restore (WO-5), Scheduler, Encryption.
- **File baru (6):** `src/main/domain/backup/domain-error.ts` (`BackupDomainError`), `backup-status.ts` (`BACKUP_STATUS` SUCCESS/SUCCESS_WITH_WARNING/FAILED/CANCELLED), `backup-session.ts` (`BackupSessionId`/`BackupSession`); `src/main/infrastructure/backup/schema-version.reader.ts` (`SchemaVersionReader.read()` dari migration terakhir applied - tanpa migration → throw), `manifest-builder.ts` (`ManifestEntrySource`, `providerKindToManifestKind`, `buildManifestPayloadJson`, `computeManifestSha256`, `ManifestBuilder.build` - meta `{backupVersion:1, type:'full', integrity:'ok', engine}`, summary `{files,totalBytes}`, checksum per entri), `packager.ts` (`BackupPackager.package` - ZIP `adm-zip` berisi `manifest.json` + staging files; `MANIFEST_FILENAME`, `APBACKUP_EXTENSION='.apbackup'`), `verifier.ts` (`BackupVerifier({tempDir}).verify(containerPath)` rantai 7 cek: file ada → ZIP+manifest → JSON+Validator → manifestSha256 payload → per-entri size+sha256 → `PRAGMA integrity_check` DB → containerSha256), `backup.service.ts` (**orkestrator** - `BackupService({providerRegistry, schemaVersionReader, manifestBuilder, packager, verifier, paths, providerStagingDirs})`; `run({appVersion, appName, engine?, isCancelled?})` → `BackupRunResult`; single-flight `BKP-<uuid>`; preflight → collect (required dulu, lalu optional) → staging via `resolveStagedPath` → build manifest → package → **verify round-trip wadah staging** → finalisasi copy ke `backup/manual/APLibrary-backup-YYYYMMDD-HHmmss-<8>.apbackup` → cleanup; `runPreflight()` database-integrity + target-directory + decideProviderPreflight; `buildBackupFilename` pure).
- **Dimodifikasi (1):** `electron/main/bootstrap.ts` - Container +`backupService` + `providerStagingDirs: Map([[databaseProvider.id.fullName, paths.tempDir]])`.
- **Validation PASS:** lint; build (main **2,016.82 kB** - preload 9.95 identik - renderer 1,147.66 identik); smoke `wo4_backup_smoke` **73/73** fresh DB (pure: `buildBackupFilename`, kind mapping, single-flight; DB-backed: reader, builder, packager round-trip, verifier rantai tamper 6 kasus, service SUCCESS/CANCELLED/SUCCESS_WITH_WARNING/FAILED/single-flight); `prisma migrate diff` empty; grep bundle `aplibrary-backup`/`manifestSha256`/`BackupService` = ter-wire (beda WO-2 domain murni 0 match).
- **Laporan:** `WORK_ORDER_4_BACKUP_ENGINE.md`. Status: **DONE - READY review PO**.
- **Commit:** `d9e9083` (+ laporan, + `.gitignore` anchor `/backup/` agar source backup ter-track; AGENTS.md update menyusul di WO-5).

### Pelajaran (retain)
- **Satu-satunya titik tulis final = service** (`copyFileSync` ke `backupManualDir` setelah verify wadah staging lulus) - bukan packager; verifier dipanggil dua arah (staging + final).
- **`providerStagingDirs`** - kontrak Provider (WO-3) tidak menyediakan jalur staging; pemetaan `providerId.fullName → stagingDir` disediakan wiring (bootstrap) dan di-resolve aman (`resolveStagedPath` guard path traversal).
- **Failure Strategy K5**: required provider gagal → FAILED; optional gagal → dilewati → SUCCESS_WITH_WARNING (manifest menandai); pembatalan antar-fase → CANCELLED tanpa file permanen.
- **`buildManifestPayloadJson` TANPA `checksums`** - `manifestSha256` dihitung atas payload tanpa self-checksums (anti-rekursif); validator (WO-2) mengisi ulang saat verify.
- **Single-flight per engine + cleanup try/finally** - satu run aktif per engine; provider & staging selalu dibersihkan.
- `adm-zip` dependency baru ditambahkan WO-4 (dipakai packager; Restore WO-5 reuse untuk extract).

---

## WO-5 (Restore Engine): RestoreService orchestrator + DatabaseRestoreHandler + schema gate + snapshot aman + wiring Container (COMPLETE - READY review PO)

### Ringkasan
- Source of Truth: ADR-001 (FINAL APPROVED, SSOT) + RFC-002/003/004. Scope: **Restore Engine lengkap** - orkestrator `RestoreService` + `DatabaseRestoreHandler` (sisi kebalikan DatabaseProvider) + util filesystem + domain restore + wiring penuh ke Container. **TANPA** UI, IPC, handler asset/log, Align/migrasi-on-restore, relokasi DB ke userData (ADR-001 §8.2 Q2-Q5).
- **File baru (7 + 1):** `src/main/domain/restore/restore-status.ts` (`RESTORE_STATUS` SUCCESS/FAILED/CANCELLED + `isRestoreStatus`), `domain-error.ts` (`RestoreDomainError`); `src/main/infrastructure/restore/schema-compare.ts` (`compareSchemaVersions` murni - same/older/newer/unknown, pola `^\d{8,}_`), `fs-utils.ts` (`resolveWithin` anti-traversal, `moveFilePreserving` rename+copy fallback, `removeSideFiles` -wal/-shm), `database-restore.handler.ts` (`DatabaseRestoreHandler` implements `RestoreHandler`+`SafeSnapshotCapable`, `isSafeSnapshotCapable` type guard; stage/verifyStaged [size+sha256+`PRAGMA integrity_check` staging]/`captureSafeSnapshot` [VACUUM INTO live via `getPrisma()`]/swapToLive [**satu-satunya titik tulis live**: disconnect → removeSideFiles → arsip live → move staging → reconnect; best-effort pulih dari arsip saat gagal]/restoreFromSafeSnapshot/cleanup), `restore.service.ts` (**orkestrator** - `createRestoreDirs(tempDir)` → root `temp/restore` + intake/extract/stage/archive/snapshot; `RestoreService({verifier, schemaVersionReader, handlerRegistry, paths, liveDatabaseFile})`; `run({backupFilePath, isCancelled?})` → `RestoreRunResult`; pipeline RFC-004 §4/§5: single-flight `RST-<uuid8>` → intake → **gate awal** [verifier.verify; backupVersion>1 ditolak; tepat satu entri DATABASE] → extract [resolveWithin] → stage+verify per entri → **schema gate** [newer → downgrade ditolak; older → Align belum didukung; unknown → ditolak] → **snapshot aman WAJIB** [VACUUM INTO; required tanpa capability → ditolak] → **swap** [gagal → rollbackAll] → **post-verify** [integrity + `_prisma_migrations` == schema backup via PrismaClient point-ke-live; gagal → rollbackAll] → SUCCESS needsRestart → finally cleanup + snapshot dipertahankan ke `temp/restore-snapshots/<sessionId>.db`); `src/main/infrastructure/database-path.ts` (`resolveLiveDatabaseFile(databaseUrl, schemaDir?)` murni - parse URL `file:` absolut Win/POSIX/relatif vs schemaDir/cwd; non-`file:` → fallback; DB belum direlokasi).
- **Dimodifikasi (2):** `electron/main/bootstrap.ts` - Container +`restoreHandlerRegistry`/`databaseRestoreHandler`/`restoreService`; `createContainer(paths, restoreWiring?)` (opsional injectable: `liveDatabaseFile` + disconnect/reconnect default `disconnectPrisma()+closeDatabase()` / `connectPrisma()+initDatabase()`); `electron/main/index.ts` - wiring nyata `createContainer(infra.paths, {...})` (liveDatabaseFile dari `resolveLiveDatabaseFile(DATABASE_URL, app.getAppPath()+'/prisma')`).
- **Validation PASS:** lint; build (main **2,036.59 kB** +19.77 dari WO-4 - preload 9.95 kB identik - renderer 1,147.66 kB identik); smoke `wo5_restore_smoke` **103/103** fresh DB (`compareSchemaVersions` 10, `createRestoreDirs` 6, `resolveLiveDatabaseFile` 8, fs-utils 8, round-trip Backup→Restore SUCCESS [sessionId RST-, schemaVersionBefore/Restored === `20260803_wo2_f2a_master_data_akademik`, needsRestart, snapshot retained non-kosong], swap bukti file diganti [B hilang], idempoten, gate awal 3 [hilang/`tidak ditemukan`, non-zip/`zip`, tamper/`sha256`], schema gate 3 [newer/`downgrade`, older/`Align`, unknown/`kompatibilitas`], backupVersion 2/`tidak didukung`, CANCELLED, single-flight `sedang berjalan`); `prisma migrate diff` empty; grep bundle `RestoreService`/`DatabaseRestoreHandler`/`resolveLiveDatabaseFile` = ter-wire, channel `restores:*` = 0.
- **Laporan:** `WORK_ORDER_5_RESTORE_ENGINE.md`. Status: **DONE - READY review PO** (tidak lanjut WO-6 UI sesuai keputusan PO).

### Pelajaran (retain)
- **Pipeline dipimpin Service; handler hanya eksekusi per-entri** - `RestoreService` mengorkestrasi intake→gate→extract→stage→schema→snapshot→swap→post-verify→rollback→cleanup; handler menerima perintah per fase. `isSafeSnapshotCapable` membedakan handler yang bisa snapshot (database) dari yang belum.
- **Snapshot aman (VACUUM INTO) = jaringan pengaman wajib** - dibuat dari live SEBELUM swap; gagal → restore ditolak; sukses → dipertahankan (`temp/restore-snapshots/<sessionId>.db`) untuk audit/recovery manual; rollback otomatis (`restoreFromSafeSnapshot`) dipakai bila swap/post-verify gagal.
- **Schema gate dua arah** (ADR-001 §6/§9): backup lebih BARU → ditolak (forward protect, tanpa downgrade); lebih LAMA → ditolak (Align/migrasi-on-restore belum didukung v1); unknown → ditolak (jalur aman). `SchemaVersionReader` (WO-4) dibaca pada saat restore (schema live TERBARU), `compareSchemaVersions` murni diuji headless.
- **swapToLive = SATU-SATUNYA titik tulis live** (invarian ADR-001 §3.3/§7, kontrak WO-3): disconnect client → removeSideFiles (-wal/-shm) → arsip live lama → move staging → reconnect; `finally` reconnect menjamin client tidak tertinggal disconnect walau gagal.
- **Wiring disconnect/reconnect = injectable** (`RestoreWiring` di `createContainer(paths, restoreWiring?)`) - memungkinkan smoke memakai handler produksi yang sama; default produksi `disconnectPrisma()+closeDatabase()` / `connectPrisma()+initDatabase()`.
- **Post-verify memakai `PrismaClient` point-ke-live** (`datasources.db.url = 'file:'+live.replace(/\\/g,'/')`), bukan `getPrisma()` singleton (yang sudah di-disconnect saat swap) - PRAGMA integrity_check + `_prisma_migrations` terakhir == schema backup.
- **Smoke memakai `BackupService` (WO-4) asli + `BackupVerifier` asli** - round-trip Backup→Restore membuktikan pasangan penuh dua engine, bukan fixture dummy.
- **Gate awal TIDAK menyentuh live** - semua kegagalan gate (hilang/bukan zip/tamper/schema/backupVersion) membuktikan live tetap berisi marker sebelum-restore.
- Smoke WO-5: compile `--module commonjs --moduleResolution node` (tanpa bwip-js) + fresh DB temp (`file:C:/...` + `prisma migrate deploy` workdir prisma/ 4 migration) + `NODE_PATH=<repo>\node_modules`; wadah custom (`buildArchiveWithMeta`) memakai `DatabaseProvider.collect` + `ManifestBuilder.build` + `BackupPackager.package` + `computeManifestSha256` untuk backupVersion/schema artifisial.
- **AGENTS.md update WO-3 & WO-4 menyusul di WO-5** - dua commit itu (2a4af89/d9e9083) membawa laporan tapi tidak membawa entri AGENTS.md; dijaga konsisten di sini.

---

## WO-6 (Backup & Restore UI): controller layer + halaman Backup/Restore (COMPLETE - READY review PO)

### Ringkasan
- Source of Truth: ADR-001 (FINAL APPROVED, SSOT) + RFC-002/003/004 + laporan WO-1..WO-5. Scope: **UI client-only** di atas engine APPROVED. **DILARANG mengubah** BackupService/RestoreService/Provider/Manifest/Domain/schema/migration.
- **Keputusan PO:** (1) "Pilih Folder" dibatalkan — backup page menampilkan **fixed path** (`userData/backup/manual`) + tombol "Buka Folder"; (2) progress = **controller-paced INDICATIVE** — stage 1 backup = preflight ASLI engine, sisanya di-paced 350ms selama `engine.run()`; restore atomic → seluruh 8 stage di-paced.
- **File baru (5):** `src/shared/dto/backup-ui.ts` (BACKUP_UI_STAGES 7 / RESTORE_UI_STAGES 8 + DTO hasil), `src/main/services/backup-ui.service.ts` (**controller layer**: `BackupInspector` [verify engine → extract DB dari wadah ke temp → PrismaClient point-ke-snapshot → count book/member/bookCopy → cleanup; read-only, tidak pernah sentuh DB live] + `BackupUIController` [getTargetInfo/run: preflight ASLI + pacing] + `RestoreUIController` [run: 8 stage di-paced]), `electron/ipc/backup-ui.ipc.ts` (8 channel + progress via `event.sender.send`; `restoreUI:pickBackup` = dialog openFile filter `.apbackup`), `electron/preload/backup-ui.preload.ts` (`backupUIAPI` = `backupUI.*`+`restoreUI.*`; `onProgress` subscribe/unsubscribe pola member), `src/pages/backup/BackupPage.tsx` + `src/pages/restore/RestorePage.tsx`.
- **Dimodifikasi (7):** `electron/main/bootstrap.ts` (+3 controller di Container), `electron/ipc/index.ts` (+`registerBackupUIHandlers`), `electron/preload/index.ts` (+spread backupUIAPI), `src/renderer/env.d.ts`, `src/utils/navigation.ts` (`ROUTES.BACKUP`/`RESTORE`), `src/utils/labels.ts` (blok `LABELS.BACKUP_RESTORE`), `src/routes/index.tsx` (+2 rute), `src/components/layout/Sidebar.tsx` (+2 menu ikon `DatabaseBackup`).
- **Toast WAJIB, DILARANG `alert()`/`confirm()` browser** — infra NS-1 (`useNotification`); konfirmasi restore memakai `confirm()` promise danger; verifikasi grep halaman baru = 0 match (satu-satunya `confirm(` = `useNotification().confirm`).
- **Validation PASS:** smoke `wo6_backup_restore_ui_smoke` **59/59** (fresh DB + engine ASLI; getTargetInfo; seed 2B/3M/4C; BackupUIController.run SUCCESS + progress validate/1..complete/7; BackupInspector.inspect counts 2/3/4 + schemaVersion F2a + backupVersion 1; inspect missing/garbage ok=false; RestoreUIController.run round-trip SUCCESS + progress validate/1..complete/8 + pasca restore counts tetap + integritas ok; restore garbage FAILED tanpa event complete); lint PASS; build PASS (main **2,046.33 kB** · preload **10.99 kB** · renderer **1,181.23 kB** `index-BRypEzV8.js`); `prisma migrate diff` = "This is an empty migration."; grep bundle 8 channel 1/1 main+preload + marker UI renderer ter-render.
- **Laporan:** `WORK_ORDER_6_BACKUP_RESTORE_UI.md`. Status: **DONE - menunggu review PO** (tidak lanjut WO berikutnya).
- **Commit:** satu final commit + push (controller + IPC + preload + UI + wiring + smoke + laporan + AGENTS.md).

### Pelajaran (retain)
- **UI layer = controller terpisah dari engine.** BackupService/RestoreService (WO-4/5) TIDAK boleh diubah; controller (`src/main/services/backup-ui.service.ts`) membungkus engine apa adanya, membangun nilai tampilan (nama file, ukuran, durasi, counts), dan headless-testable tanpa Electron.
- **Progress indicative = race pacing vs enginePromise** (`Promise.race([engine.then(settled), sleep(350).then(not-settled)])` per stage) — stage 1 backup memanggil `runPreflight()` ASLI (keputusan nyata engine), bukan label tiruan; restore seluruhnya di-paced karena atomic (internal engine tidak boleh bocor).
- **Counts Buku/Anggota/Eksemplar TIDAK ada di manifest** (ADR-001 §8.2 Q5 open) → dihitung `BackupInspector` dari DB di dalam wadah: `BackupVerifier.verify` (rantai 7 cek engine) → extract entri DATABASE ke temp → `new PrismaClient({datasources:{db:{url:'file:'+path}}})` → count → disconnect+unlink temp (`finally`). Read-only, tidak pernah sentuh DB live.
- **Renderer TIDAK menurunkan angka** (konsisten WO-2/P-4/Dashboard/Report) — formatSize/formatDuration/formatDate hanyalah format tampilan; status/version/counts/durasi datang dari DTO main. UI hanya klien IPC.
- **Progress subscribe/unsubscribe pola member** — `onProgress` mengembalikan `() => void` (unsubscribe); cleanup `unsubscribeRef` pada unmount & setelah run mencegah leak listener saat navigasi.
- **`Promise.race` + `as const`** — guard TS: `enginePromise.then(() => ({ settled: true as const }))` vs `sleep().then(() => ({ settled: false as const }))` agar `outcome.settled` ter-narrow; pola sama di controller backup & restore.
- **Smoke WO-6 memakai engine + wadah ASLI**: `BackupUIController.run` menghasilkan `.apbackup` nyata di disk → `BackupInspector.inspect` membuktikan file yang sama terbaca → `RestoreUIController.run` mengembalikan isi backup (round-trip penuh). Bukan fixture dummy.
- **Pitfall PowerShell `$c.Split('needle')`** — `.Split(string)` memperlakukan string sebagai char[] separator (count absurd 4xx); gunakan `($c -split [regex]::Escape($needle)).Length - 1` untuk grep jumlah channel di bundle.
- Smoke compile: `--module commonjs --moduleResolution node` (tanpa bwip-js) + fresh DB temp (`file:C:/...` + `prisma migrate deploy` workdir prisma/ 4 migration) + `NODE_PATH=<repo>\node_modules`; pola konstruksi engine persis `wo5_restore_smoke/smoke.ts` (restoreWiring injectable + `connectPrisma`/`disconnectPrisma`).

---

## LOGO MANAGEMENT (WO-1 foundation + WO-2 backend + WO-2 R1 atomic save + WO-3 UI + audit) (COMPLETE - READY review PO)

### Ringkasan
- **WO-1 (foundation, commit `a541dd1`):** `src/main/infrastructure/asset/logo-config.ts` (`LOGO_BASENAME='school-logo'`, whitelist `LOGO_IMAGE_MIME` png/jpg/jpeg/webp, `MAX_LOGO_SIZE_BYTES` = 512 KB, `validateLogoFile` format→size); `paths.assetSchoolLogoDir` (assets/school-logo); `asset-resolver.ts` baseline (resolveAssetPath/cleanupLegacyLogos, `isOldAbsoluteLogoPath`); PrintService wiring assetRoot; label LOGO.
- **WO-2 (backend):** `SettingService` ctor kini `(settingRepository, assetSchoolLogoDir)` — `saveLogo(filePath)` [validasi format/size → resize sharp → tulis temp `.${basename}.tmp-<uuid>` dot-prefix di dalam `assetSchoolLogoDir` → `settings.update({logoPath})` → cleanup file lama], `clearLogo()`, `pickLogoPreview(filePath)`; `src/main/infrastructure/asset/logo-resize.ts` (`resizeLogoImage` via sharp, `LOGO_RESIZE_MAX_DIMENSION=512`, **downscale-only contain** — ≤512×512 → byte asli tanpa proses); `src/shared/dto/logo.ts` (`PickLogoResult {canceled:true} | {canceled:false,filePath,sizeBytes,previewUri}`); IPC `settings:pickLogo` (dialog OS + validasi + resize + preview di main, `BrowserWindow.fromWebContents`); preload `settings.pickLogo`; `env.d.ts`; `electron-builder.yml` asarUnpack `node_modules/@img/**`+`node_modules/sharp/**`; `electron.vite.config.ts` external `sharp`; CSP renderer `img-src ... data:` (previewUri).
- **WO-2 R1 (atomic save):** urutan disempurnakan — tulis temp baru dulu → DB commit → hapus file lama; kegagalan DB → temp dibersihkan + file lama aman (invariant RFC §10 dipenuhi). Audit fault-injection `wo2_logo_audit_sim` (harness pra-R1, TIDAK di-commit) mendokumentasikan pelanggaran invariant sebelum R1.
- **WO-3 (UI preview):** `SettingsPage.tsx` tab Identitas — preview 96×96 + nama file + ukuran + tombol Pilih Logo (spinner saat picking); payload `logoUpload` dikirim di `settings.update`.
- **TIDAK diubah:** schema/migration (`prisma migrate diff` = empty), Repository, Borrow/Return/Print pipeline, Backup/Restore, Dashboard, Report, `Setting.barcodeFormat` (tidak dikonsumsi).
- **Validation PASS:** lint; build (main **2,066.78 kB** · preload **11.54 kB** · renderer **1,216.08 kB**); smoke `wo2_logo_backend_smoke` **42/42** · `wo2_r1_atomic_save_smoke` **56/56** (fault-injection write-temp/move/DB gagal + invariant `.tmp-`/`.old-` bersih) · `wo4_logo_save_smoke` **20/20**; regression borrow_card_uat **31/31** + wo2_borrow_card_preview **21/21** (fix ctor SettingService 2-arg); bundle grep `settings:pickLogo`/`saveLogo`/`school-logo` ter-render.
- **Laporan:** `RFC_LOGO_MANAGEMENT_ARCHITECTURE.md`, `LOGO_MANAGEMENT_DISCOVERY.md`, `WORK_ORDER_LOGO_2_BACKEND.md`, `WORK_ORDER_LOGO_2_REVISION1_ATOMIC_SAVE.md`, `WORK_ORDER_LOGO_3_UI_PREVIEW.md`, `WORK_ORDER_LOGO_2_AUDIT.md`. Status: **DONE - READY review PO**.

### Pelajaran (retain)
- **`SettingService` ctor berubah 2-arg** (`assetSchoolLogoDir`) — semua pemanggil termasuk smoke regression (`borrow_card_uat_smoke`, `wo2_borrow_card_preview_smoke`) wajib di-update; UI memakai `settings.update({ logoUpload: filePath })` (file path absolut), BUKAN channel terpisah.
- **Temp file ber-prefix DOT (`.school-logo.webp.tmp-<uuid>`)** — di dalam `assetSchoolLogoDir`, lalu rename/move ke nama final saat sukses; invariant smoke menyaring nama `.tmp-`/`.old-` dan memastikan tidak ada sisa setelah sukses/gagal. Injection write-temp yang benar = patch `fs.promises.writeFile` (path mengandung `.tmp-`), BUKAN membuat direktori tanpa leading dot.
...
## PRINT-DEBUG-ORIGIN-1 (Debug Origin Cetak): Halaman Uji 110x60mm — REVISI DUA MODE (COMPLETE - READY review PO)

### Ringkasan
- **WO:** halaman debug untuk menemukan layer yang menggeser seluruh halaman cetak ke kiri (reproduksi issue kartu peminjaman terpotong di kiri). READ ONLY + DEBUG - TIDAK mengubah pageSize/margin/driver/PrintService pipeline kartu.
- **Revisi review PO:** desain awal hanya 1 mode (memakai `PRINT_OFFSET_PT` via marginType custom left) **DITOLAK** — eksperimen tidak valid karena tidak bisa membedakan pipeline aplikasi vs driver/printer. Desain direvisi menjadi **DUA MODE**:
  - **MODE A (PURE)** — `printDebugGridPure()`: `margins: { marginType: 'none' }`, origin 0, **TANPA** `PRINT_OFFSET_PT`, TANPA offset, TANPA workaround.
  - **MODE B (PRODUCTION)** — `printDebugGrid()`: opsi **IDENTIK `printBorrowCard`** — margins custom `left=PRINT_OFFSET_PT` + pageSize 110×60.
  - **Tujuan:** PO mencetak PURE lalu PRODUCTION di printer kartu yang sama → PURE ikut bergeser = bug driver/printer; hanya PRODUCTION bergeser = bug pipeline aplikasi.
- **Konten halaman (persis mandat):** (1) border merah 1px keliling page; (2) tulisan "(0,0)" pojok kiri atas; (3) grid tiap 5mm; (4) 4 kotak hitam 30x30 di sudut; (5) koordinat X=0/20/40/60/80/100 & Y=0/20/40.
- **File baru (4):** src/main/services/print-debug.service.ts (generatePrintOriginDebugHtml() PURE - SVG 110x60 viewBox, tanpa DB/Electron, @page 110mm 60mm margin 0), src/pages/PrintOriginDebugPage.tsx (preview iframe srcDoc ukuran asli 110x60mm + **DUA tombol Cetak** — Cetak PURE (A) & Cetak PRODUCTION (B) + hint; toast via useNotification, TANPA alert/confirm), print_debug_origin_smoke/smoke.ts (**21/21 PASS** murni), print_debug_origin_smoke/main.cjs (**15/15 PASS** Electron dua mode).
- **Dimodifikasi (6):** electron/main/services/print.service.ts (+getDebugGridPreviewHtml + printDebugGridPure [MODE A PURE] + printDebugGrid [MODE B PRODUCTION]), electron/ipc/print.ipc.ts (+printing:debugOriginPreview/printing:debugOriginPure/printing:debugOrigin), electron/preload/print.preload.ts (+print.debugOriginPreview/print.debugOriginPure/print.debugOrigin), src/renderer/env.d.ts, src/routes/index.tsx (+debug/print-origin), src/utils/navigation.ts (+DEBUG_PRINT_ORIGIN), src/utils/labels.ts (+blok PRINT_DEBUG), src/components/layout/Sidebar.tsx (+item "Debug Cetak" ikon Bug).
- **TIDAK diubah:** schema/migration (prisma migrate diff = "This is an empty migration."), PrintService printBorrowCard/saveBorrowCardPdf/printHtml, renderPdf, template kartu, layout, business logic, DTO.
- **Validation PASS:** lint; build (main **2,071.12 kB** - preload **11.79 kB** - renderer **1,225.49 kB** index-C0SgWsAo.js); smoke murni **21/21**; smoke Electron dua mode **15/15** (PURE marginType=none + tanpa pageSize + tanpa custom left; PRODUCTION custom left=8.5 + pageSize 110000×60000; **Eksperimen valid: PURE != PRODUCTION**); prisma migrate diff empty; grep bundle main printing:debugOriginPure=1/printing:debugOrigin=3, preload debugOriginPure=2, renderer debug/print-origin=3/Cetak PURE=2/Cetak PRODUCTION=2.
- **Laporan:** WORK_ORDER_PRINT_DEBUG_ORIGIN_1.md (rencana). Status: **DONE - READY review PO** (UAT printer di perangkat PO, tidak membuka WO baru).

### UAT (langkah PO di mesin dengan printer label/kartu)
1. Jalankan aplikasi -> Sidebar menu **Debug Cetak**.
2. Klik **Cetak PURE (A) — tanpa offset** -> dialog printer -> pilih printer yang dipakai PO untuk kartu peminjaman.
3. Klik **Cetak PRODUCTION (B) — pipeline kartu** -> dialog printer yang sama.
4. Bandingkan kedua lembar: PURE bergeser/terpotong kiri juga? Hanya PRODUCTION yang bergeser? Ukur pergeseran mm (grid 5mm + koordinat X/Y).
5. **Kesimpulan:** PURE ikut bergeser → bug driver/printer (gutter non-printable). Hanya PRODUCTION bergeser → bug pipeline aplikasi (offset/pageSize).

### Pelajaran (retain)
- **Eksperimen debug WAJIB DUA MODE — PURE + PRODUCTION.** Satu mode yang memakai `PRINT_OFFSET_PT` (marginType custom left) tidak bisa menjawab "driver vs pipeline" karena offset aplikasi menutupi gejala driver. MODE A (PURE) `marginType: 'none'` tanpa pageSize/offset/workaround = baseline origin 0; MODE B (PRODUCTION) = opsi identik `printBorrowCard` agar gejala 1:1 ter-reproduksi. Eksperimen VALID bila opsi PURE ≠ PRODUCTION (dibuktikan smoke).
- **Preview debug dirender di iframe srcDoc**, bukan dangerouslySetInnerHTML - HTML debug adalah full document (@page + html/body margin 0) yang akan mengacaukan tata letak halaman aplikasi bila di-inline.
- **SVG 110x60 dengan width/height mm + viewBox "0 0 110 60"** menjadikan 1 unit SVG = 1mm - koordinat (0,0)/kotak/grid semua dalam mm, PO bisa mengukur pergeseran dengan penggaris.
- **Grep bundle channel**: main printing:debugOriginPure=1 (1 call-site), printing:debugOrigin=3 (debugOriginPreview+debugOriginPure+debugOrigin), preload debugOriginPure=2 (key + string channel) - konsisten pola R-4 (grep string channel, bukan bentuk properti).
- Smoke murni compile --module commonjs --moduleResolution node (tanpa bwip-js) + run plain node; count(html,'<line ')===32 (21 vertikal + 11 horizontal, tepi 0 = border merah). Smoke Electron dua mode compile --module node16 --moduleResolution node16 (bwip-js transitif via print.service) + outDir DI DALAM repo (Electron abaikan NODE_PATH) + `electron main.cjs <outDir>`.

---

## PRINT-DEBUG-ORIGIN-2 (Debug Origin Cetak): MODE C mengisolasi pageSize (COMPLETE - READY review PO)

### Ringkasan
- **WO:** REVISI UAT PRINT-DEBUG-ORIGIN-1 — hasil fisik MODE A (PURE) & MODE B (PRODUCTION) SAMA-SAMA bergeser/terpotong, perbedaan tidak signifikan. Review PO: eksperimen tidak sepenuhnya mengisolasi variabel karena A (margin none, tanpa pageSize) vs B (custom left + pageSize 110×60) mengubah **pageSize DAN margin sekaligus**. WO ini menambah **MODE C (SIZE)** untuk mengisolasi pengaruh pageSize. READ ONLY + DEBUG.
- **MODE C HARUS:** `margins: { marginType: 'none' }` + `pageSize: { width: 110000, height: 60000 }`; TANPA `PRINT_OFFSET_PT` / custom left / transform / scale / workaround; HTML debug **IDENTIK** MODE A & B.
- **Perbandingan 3 arah:** A (none, pageSize default) vs C (none, 110×60) → isolasi **pageSize**; C (none, 110×60) vs B (custom, 110×60) → isolasi **margin offset**.
- **File diubah (4 source + 1 smoke):** `electron/main/services/print.service.ts` (+`printDebugGridSize()` MODE C — margins none + pageSize dari BORROW_CARD_LAYOUT ×1000, komentar perbandingan 3 arah), `electron/ipc/print.ipc.ts` (+`printing:debugOriginSize`), `electron/preload/print.preload.ts` (+`print.debugOriginSize`), `src/renderer/env.d.ts` (+`debugOriginSize`), `src/pages/PrintOriginDebugPage.tsx` (state busySize + `handlePrintSize()` + tombol ke-3 "Cetak SIZE (C) — pageSize 110×60", grid sm:grid-cols-3, hint SIZE), `src/utils/labels.ts` (`PRINT_SIZE`, `HINT_COMPARE_SIZE`, HINT_TITLE → "ketiga mode", HINT_COMPARE_PURE/SIZE/PROD), `print_debug_origin_smoke/main.cjs` (3-mode).
- **TIDAK diubah:** `printBorrowCard` production, CSS debug, ukuran 110×60, driver, konfigurasi Windows, MODE A/B dihapus, schema/migration, routes/navigation/Sidebar (halaman debug sudah ada).
- **Validation PASS:** lint; build (main **2,072.42 kB** +1.30 · preload **11.88 kB** +0.09 · renderer **1,227.01 kB** `index-BmeT8pU1.js` +1.52); smoke murni **21/21**; smoke Electron 3-mode **21/21** (PURE none/no pageSize · **SIZE none + 110000×60000 tanpa custom left** · PRODUCTION custom 8.5 + 110×60; eksperimen valid: SIZE punya pageSize ≠ PURE, SIZE tanpa custom left ≠ PRODUCTION); grep bundle main `printing:debugOriginSize`=1, preload `debugOriginSize`=2, renderer `Cetak SIZE`=2/`debugOriginSize`=1.
- **Laporan:** WORK_ORDER_PRINT_DEBUG_ORIGIN_1.md (rencana). Status: **DONE - READY review PO** (UAT printer di perangkat PO, tidak commit/push).

### UAT (langkah PO di mesin dengan printer label/kartu) — cukup MODE C SATU KALI
1. Jalankan aplikasi -> Sidebar menu **Debug Cetak**.
2. Klik **Cetak SIZE (C) — pageSize 110×60** -> dialog printer yang sama seperti UAT sebelumnya.
3. Bandingkan lembar C dengan hasil A (PURE) dan B (PRODUCTION) yang sudah dicetak:
   - **C mirip A (tidak bergeser), B bergeser** → bug dari **margin custom left (PRINT_OFFSET_PT)**, bukan pageSize. Arah perbaikan: nilai/knob offset.
   - **C mirip B (bergeser), A tidak** → bug dari **pageSize 110×60**. Arah perbaikan: perlakuan pageSize oleh driver.
   - **Ketiganya bergeser** → bug driver/printer (gutter non-printable) — offset aplikasi tidak relevan.
   - **A & C bergeser tapi B berbeda** → kombinasi; dokumentasikan mm per lembar (grid 5mm).

### Pelajaran (retain)
- **Eksperimen mengisolasi SATU variabel per langkah, bukan dua.** A vs B mengubah pageSize + margin sekaligus → ambiguitas. MODE C (none + pageSize 110×60) memotong diagonal: A↔C menyekat pageSize, C↔B menyekat offset. Tanpa MODE C, kesimpulan "driver vs pipeline" tidak sah secara eksperimen.
- **MODE C memakai SSOT yang sama**: `pageSize` dibangun dari `BORROW_CARD_LAYOUT.pageWidthMm/HeightMm × 1000` (bukan hardcode 110000/60000) — satu sumber dimensi kartu, konsisten pola printBorrowCard.
- **MODE C TIDAK boleh menyalin apa pun dari MODE B** selain pageSize: margins tetap `{ marginType: 'none' }`, tanpa custom left — bukti smoke `SIZE TANPA margins custom left` + `SIZE pageSize = 110000×60000`.
- **Intercept webContents.print tetap pola lama** (patch loadURL → wc.print spy) — tanpa mengubah PrintService; urutan tangkapan = PURE, SIZE, PRODUCTION (captured=1/2/3).
- **Grep bundle channel**: main `printing:debugOriginSize`=1 (handler), preload `debugOriginSize`=2 (key + string channel), renderer `debugOriginSize`=1 (invoke) + `Cetak SIZE`=2 (label + button) — pola R-4.
- Smoke Electron compile --module node16 --moduleResolution node16 (bwip-js transitif via print.service) + outDir DI DALAM repo (Electron abaikan NODE_PATH) + `electron main.cjs <outDir>`; hapus folder `out/` hasil compile setelah run.

---

## PRINT-DEBUG-PDF-1 (Debug Origin Cetak): Diagnostik PDF 110x60mm — reuse renderPdf (COMPLETE - READY review PO)

### Ringkasan
- **WO:** fungsi diagnostik yang menghasilkan **PDF 110x60mm** dari HTML debug origin yang SAMA PERSIS (generatePrintOriginDebugHtml, @page 110mm 60mm margin 0). **READ ONLY terhadap production behavior**: printBorrowCard / PRINT_OFFSET_PT / pageSize cetak / driver / opsi Electron TIDAK diubah. **Reuse mekanisme PDF yang ada** (PrintService.renderPdf -> printToPDF preferCSSPageSize:true, WO BORROW CARD PDF FIX) — TIDAK ada engine PDF kedua, TIDAK ada offset/transform/scale/workaround/penyesuaian driver.
- **Modifikasi (4 source + 1 smoke + 1 UI):** electron/main/services/print.service.ts (+buildDebugPdfFilename() pure `'Debug Origin 110x60 - PDF.pdf'` + saveDebugPdf() = generatePrintOriginDebugHtml -> renderPdf -> dialog.showSaveDialog -> writeFile -> {saved,filePath?}), electron/ipc/print.ipc.ts (+printing:debugOriginPdf), electron/preload/print.preload.ts (+print.debugOriginPdf), src/renderer/env.d.ts (+debugOriginPdf), src/pages/PrintOriginDebugPage.tsx (+tombol "Simpan PDF (D) - 110x60" ikon FileDown + handleSavePdf toast), src/utils/labels.ts (+PRINT_PDF/PDF_SAVING/PDF_SAVED/PDF_ERROR/HINT_PDF), print_debug_pdf_smoke/main.cjs (BARU).
- **TIDAK diubah:** printBorrowCard, PRINT_OFFSET_PT, printHtml, mode debug A/B/C, renderPdf signature, template kartu, schema/migration (WO murni diagnostik).
- **Validation PASS:** smoke Electron `print_debug_pdf_smoke` **16/16** (real renderPdf pada HTML debug: MediaBox [0 0 312 169.92]pt = 110.067x59.944mm PASS, 1 halaman, content stream ter-dekompresi 79168 chars, rect border 110x60 terdeteksi, 4 kotak hitam 30x30 terdeteksi, posisi kotak tepat di 4 sudut `0,0|0,30|80,0|80,30` — SVG 1 unit = 1mm terbawa ke content stream PDF; buildDebugPdfFilename murni; kontrol tanpa preferCSSPageSize = A4 792x612pt); lint PASS; build PASS (main 2,025.44 kB · preload 11.96 kB · renderer 1,228.75 kB); grep bundle printing:debugOriginPdf main 1, debugOriginPdf preload 2, `Simpan PDF` renderer 3.
- **Ukuran PDF:** 41,708 bytes (MediaBox 312.000x169.920pt, Chromium membulatkan ke kelipatan 0.08pt — persis pola kartu peminjaman).
- **Laporan:** WORK_ORDER_PRINT_DEBUG_PDF_1.md. Status: **DONE - READY review PO** (tidak commit/push, menunggu review).

### Pelajaran (retain)
- **Reuse renderPdf existing = nol parameter tambahan.** Karena HTML debug sudah ber-`@page { size: 110mm 60mm; margin: 0; }` dan renderPdf memakai preferCSSPageSize:true, memanggil renderPdf(generatePrintOriginDebugHtml()) langsung menghasilkan halaman 110x60mm — TANPA menyentuh opsi printToPDF, pageSize, margins, offset. Ini membuktikan pipeline PDF existing benar sejak awal; masalah pergeseran kiri hanya ada di jalur cetak fisik.
- **Content stream PDF memakai satuan mm untuk SVG viewBox** (SVG 1 unit = 1mm terbawa ke operator `re` PDF): border 110x60, kotak 30x30 di (0,0)/(80,0)/(0,30)/(80,30). Dua rect ekstra besar (416x227, 1299x709) adalah clip rect layer SVG, bukan konten debug — jangan assert `rects.length === 5`.
- **Validasi PDF via dekompresi FlateDecode** (zlib.inflateSync per `stream...endstream`) lalu regex operator `re` (x y w h re) — memverifikasi KONTEN nyata (border + 4 kotak + posisi sudut), bukan hanya MediaBox. Ini pelengkap pola WO BORROW CARD PDF FIX (hanya MediaBox).
- **Save-dialog tidak bisa diuji headless** — smoke menguji renderPdf ASLI + konten; kontrak `{saved,filePath?}` dan handler IPC dibuktikan via grep bundle. UI button adalah jalur PO.
- **Nama file PDF diagnostik = helper PURE** (buildDebugPdfFilename) agar bisa diuji tanpa Electron, pola buildBorrowCardPdfFilename.
- Smoke compile & run mengikuti pola WO-2/preview: --module node16 --moduleResolution node16 (bwip-js transitif via print.service) + outDir DI DALAM repo (Electron abaikan NODE_PATH) + `electron main.cjs <absOutDir> <pdfPath>`; hapus folder `out/` hasil compile setelah run.

---

## PRINT-CARD-SIZE-110X86-TEST-1 (Debug Origin Cetak): MODE S menguji ukuran kartu baru 110×86 (COMPLETE - READY review PO)

### Ringkasan
- **WO:** EKSPERIMEN TERISOLASI untuk menguji `pageSize` **110×86 mm** — TARGET UKURAN KARTU BARU hasil keputusan PO (kartu produksi akan diganti 110×60 → 110×86 sesuai hasil eksperimen ini). Kedua dimensi memenuhi minimum driver EPSON L3110 (width 110≥54mm, height 86≥86mm). READ ONLY + DEBUG.
- **MODE S HARUS:** `margins: { marginType: 'none' }` + `pageSize: { width: 110000, height: 86000 }` mikron; TANPA `PRINT_OFFSET_PT` / custom left / transform / scale / workaround; konten = pola debug 110×86 (border, (0,0), grid 5mm, 4 kotak 30×30 di sudut y=0/56, koordinat X=0..100 / Y=0..80).
- **Parameterisasi:** `buildDebugPatternMarkup()` → `buildDebugPatternMarkup(widthMm = 110, heightMm = 60)` — panggilan default **byte-identik** dengan baseline (origin 3449 / rotated 3653 chars), dibuktikan smoke (tanpa duplikasi pola markup antar fungsi).
- **File diubah (4 source + 1 UI + 2 smoke):** `src/main/services/print-debug.service.ts` (parameterisasi + `generateSize86DebugHtml()` — @page 110mm 86mm margin 0, svg `width="110mm" height="86mm" viewBox="0 0 110 86"`, `.debug-sheet` 110×86), `electron/main/services/print.service.ts` (+`getDebugSize86PreviewHtml()` +`printDebugGridSize86()` margins none + pageSize 110000×86000), `electron/ipc/print.ipc.ts` (+`printing:debugSize86Preview`/`printing:debugSize86`), `electron/preload/print.preload.ts` (+`print.debugSize86Preview`/`debugSize86`), `src/renderer/env.d.ts`, `src/pages/PrintOriginDebugPage.tsx` (kartu hijau-emerald "Eksperimen Ukuran Kartu Baru (110×86)" + tombol "Cetak SIZE86 (S)" + preview iframe 110mm×86mm + hint), `src/utils/labels.ts` (+`PRINT_SIZE86`/`SIZE86_TITLE`/`SIZE86_SUBTITLE`/`SIZE86_HINT`/`SIZE86_HINT_RESULT`/`HINT_SIZE86`), `print_debug_origin_smoke/smoke.ts` (section 11 MODE S), `print_debug_origin_smoke/main.cjs` (MODE S `capturedPrints[4]` + HTML preview + eksperimen valid 2 report).
- **TIDAK diubah:** `printBorrowCard` production, `PRINT_OFFSET_PT`, `printHtml`, mode A/B/C/R, `renderPdf`, template kartu, `borrow-card.service.ts`, driver, schema/migration (WO diagnostik), routes/navigation/Sidebar.
- **Validation PASS:** lint; build (main **2,078.13 kB** · preload **12.30 kB** · renderer **1,237.84 kB**); smoke murni **57/57** (21 lama + 36 MODE S: grid 38 garis = 21 vertikal + 17 horizontal, kotak y=56, X=0..100 / Y=0..80, viewBox 110 86, deterministik, `!includes(110x60 pattern)` = tidak duplikasi); smoke Electron 5-mode **40 report PASS** (PURE none/no pageSize · SIZE none + 110000×60000 · PRODUCTION custom 8.5 + 110×60 · ROT none + 60000×110000 · **SIZE86 none + 110000×86000 tanpa custom left**; eksperimen valid: SIZE86 pageSize 110×86 = target baru, width sama PROD height beda, TANPA custom left sama PURE/SIZE/ROT beda PROD); grep bundle main `printing:debugSize86`=2, preload `debugSize86`=4, renderer `Cetak SIZE86`=2/`Eksperimen Ukuran Kartu Baru`=1.
- **Laporan:** `WORK_ORDER_PRINT_CARD_SIZE_110X86_TEST_1.md`. Status: **DONE - READY review PO** (UAT printer di perangkat PO, tidak commit/push).

### UAT (langkah PO di mesin dengan printer label/kartu) — cukup MODE S SATU KALI
1. Jalankan aplikasi -> Sidebar menu **Debug Cetak**.
2. Klik **Cetak SIZE86 (S) — pageSize 110×86** -> dialog printer yang sama.
3. Bandingkan lembar S dengan hasil mode lain yang sudah dicetak:
   - **SIZE86 tercetak utuh tanpa terpotong kiri** → ukuran 110×86 SAH sebagai kartu produksi baru → arah perbaikan = ubah kartu 110×60 → 110×86.
   - **Tetap terpotong** → masalah lain (bukan height minimum).

### Pelajaran (retain)
- **Parameterisasi fungsi yang dipakai banyak mode = default arg + bukti byte-identity.** `buildDebugPatternMarkup(widthMm=110, heightMm=60)` memakai parameter dengan default yang menghasilkan output IDENTIK; smoke lama harus tetap hijau (origin/rotated byte-sama). Geometri baru (110×86) hanya menambah call-site — jangan duplikasi pola markup.
- **MODE S = target kartu baru, bukan workaround**: keduanya memenuhi minimum driver (width 110≥54, height 86≥86mm) sehingga hasil cetak menentukan keputusan PO mengubah ukuran produksi. MODE R (60×110) adalah jembatan sementara; MODE S adalah keputusan akhir.
- **Geometri s86 deterministik:** grid = vertikal x=5..105 (21) + horizontal y=5..85 (17) = 38 garis; kotak bawah pada `y = heightMm-30 = 56`; label X loop `x < widthMm` step 20 (100 terakhir, anchor end karena `100+20 >= 110`); label Y loop `y < heightMm` → 0..80. Verifikasi formula, bukan hardcode.
- **Intercept webContents.print tetap pola lama** (patch loadURL → wc.print spy) — urutan tangkapan = PURE, SIZE, PROD, ROT, SIZE86 (captured=1..5).
- **Grep bundle channel**: main `printing:debugSize86`=2 (2 handler), preload `debugSize86`=4 (2 key + 2 channel), renderer `Cetak SIZE86`=2 (label + button) + `Eksperimen Ukuran Kartu Baru`=1 — pola R-4.
- Smoke compile: murni `--module commonjs --moduleResolution node` + node; Electron `--module node16 --moduleResolution node16` (bwip-js transitif via print.service) + outDir DI DALAM repo + `electron main.cjs <outDir>`; hapus folder `out/` hasil compile setelah run.

---

## PRINT-DRIVER-MEDIA-110X86-TEST-1 (Debug Origin Cetak): Eksperimen media fisik 110×86 — konfirmasi MODE S (COMPLETE - READY review PO)

### Ringkasan
- **WO:** menegaskan eksperimen media fisik **110×86 mm** (target kartu baru PO; width 110≥54, height 86≥86mm minimum EPSON L3110). Eksperimen yang diminta **SUDAH ada sebagai MODE S (SIZE86)** dari `WORK_ORDER_PRINT_CARD_SIZE_110X86_TEST_1.md` — WO ini **tanpa perubahan kode baru**, hanya konfirmasi + pelaporan pemenuhan mandat (READ ONLY production).
- **EXACT PRINT REQUEST (dibuktikan smoke Electron intercept `webContents.print`):** `S86_OPTS={"margins":{"marginType":"none"},"pageSize":{"width":110000,"height":86000}}` — width 110000 µm, height 86000 µm, margins none, TANPA offset (`margins.left` undefined, `marginType != 'custom'`).
- **PRODUCTION ISOLATION:** `printBorrowCard` tetap `margins custom left=8.5 + pageSize 110000×60000` (smoke `PROD_OPTS`), `BORROW_CARD_LAYOUT` tetap 110×60, `PRINT_OFFSET_PT` tidak dipakai MODE S; seluruh perubahan print.service.ts aditif (mode debug). 5 mode berbeda dibuktikan (PURE≠SIZE≠PROD≠ROT≠SIZE86), SIZE86 tanpa custom left.
- **CONTENT:** `generateSize86DebugHtml()` — border merah, `(0,0)`, grid 5mm (38 garis), 4 kotak 30×30 di y=0/56, koordinat X=0..100 / Y=0..80, `@page 110mm 86mm margin 0`, viewBox 110 86. Menjawab: konten di dalam media / origin benar / ada-tidak pergeseran X/Y / ukuran fisik.
- **Validation PASS:** lint; build (main 2,078.13 kB · preload 12.30 kB · renderer 1,237.84 kB); smoke murni **57/57**; smoke Electron **40 report PASS** (5 mode + eksperimen valid + preview HTML 3822 chars == `generateSize86DebugHtml`).
- **Laporan:** `WORK_ORDER_PRINT_DRIVER_MEDIA_110X86_TEST_1.md`. Status: **DONE - READY review PO** (UAT printer di perangkat PO, tidak commit/push).
- **Pelajari pola:** mode test baru untuk eksperimen ukuran media = tambahkan mode debug di `print-debug.service.ts` (parametrisasi default byte-identik) + `print.service.ts` (margins none + pageSize mikron) + IPC/preload/env + halaman + smoke murni & Electron; selalu intercept `webContents.print` untuk membuktikan EXACT REQUEST (bukan hanya kode).

---

## FILE MENU (REVISI 4: WINDOW CONTROLS DIHAPUS dari TopBar + REVISI 3 Backup/Restore PINDAH ke File + NATIVE MENU DIHAPUS + Custom Dropdown + Preview-first): Backup / Restore / Cetak Kartu Peminjaman / Exit (COMPLETE - READY review PO)

### Ringkasan
- **REVISI 4 (window controls):** **tiga tombol Minimize/Maximize/Close di TopBar DIHAPUS** (fungsi helper `minimize()/maximize()/close()` + blok tombol + import `Minus/Square/X`) — ketiganya menduplikasi tombol native Windows title bar; window tetap ber-frame (`frame:false`/`titleBarStyle` tidak dipakai) sehingga title bar asli tetap punya Minimize/Maximize/Close. Bagian TopBar lain (File dropdown, logo, user, Settings, Logout) TIDAK berubah; Exit tetap di File dropdown (`window.electronAPI.window.close()` di `FileMenuDropdown.handleSelect`). Preload `window.*` (minimize/maximize/close) dibiarkan utuh — dipakai Exit & native title bar via OS.
- **REVISI 3 (lokasi akses):** **Backup & Restore DIPINDAHKAN dari Sidebar ke menu File** (TopBar) — item `{to:'/backup', label:'Backup'}` & `{to:'/restore', label:'Restore'}` + import `DatabaseBackup` dihapus dari `Sidebar.tsx`; item Sidebar lain TIDAK diubah. Halaman/route/handler/logic Backup-Restore yang SUDAH ADA dipakai apa adanya (tidak ada fitur baru); File dropdown sudah memuat keempat item (Backup/Restore/Cetak Kartu Peminjaman/Exit) dan navigasi `useNavigate(route)` → `/backup`/`/restore` membuka halaman yang sama.
- **REVISI 2 (diatas REVISI dropdown renderer):** **native application menu Electron DIHAPUS SEPENUHNYA** — `electron/main/menu.ts` di-delete, `installApplicationMenu()` dihapus, diganti SATU baris `Menu.setApplicationMenu(null)` di `electron/main/index.ts` (sebelumnya template Edit/View/Window/Help + appMenu macOS). App kini TIDAK punya menu bar native sama sekali; hanya TopBar React (File dropdown + window controls) yang ada.
- **REVISI (diatas versi native-menu asli):** menu **File** dipindah dari native Electron menu ke **dropdown kustom renderer** (`FileMenuDropdown` di TopBar) — tepat 4 item: **Backup** (→ /backup), **Restore** (→ /restore), **Cetak Kartu Peminjaman** (→ /borrowings/print-card), **Exit** (→ `window.electronAPI.window.close()`; window-all-closed → app.quit di non-darwin).
- **Arsitektur:** SATU router existing (HashRouter). Dropdown menavigasi langsung via `useNavigate(route)` — TANPA channel IPC, TANPA preload `menu`, TANPA listener renderer. Config MURNI `FILE_MENU_ITEMS` (`src/components/layout/file-menu-items.ts`: id/label/icon-string-key/route|exit) dipisah dari komponen (ikon di-map di komponen) agar smoke headless bisa mengimpor tanpa React/lucide.
- **File baru (2):** `src/components/layout/file-menu-items.ts` (config murni — sumber kebenaran item File), `src/components/layout/FileMenuDropdown.tsx` (trigger gelap + panel putih `min-w-[200px] z-[70]`, tutup saat klik-luar/Escape, Exit item merah, navigasi via `useNavigate`).
- **Modifikasi (4):** `src/utils/labels.ts` (+blok `FILE_MENU`), `src/components/layout/TopBar.tsx` (+`<FileMenuDropdown />`), `electron/main/index.ts` (`Menu.setApplicationMenu(null)` — import `Menu` + panggilan menggantikan `installApplicationMenu()`), `src/pages/BorrowCardPrintPage.tsx` (**preview-first**).
- **REVISI 2 (preview-first):** tombol "Cetak Kartu" di `BorrowCardPrintPage` → **"Lihat Preview"** (`ACTION_PREVIEW`, ikon Eye) yang menavigasi `receiptPreviewPath(row.id)`; `print.borrowCard(id)` + `printingId` + toast PRINT_SENT/PRINT_ERROR dihapus dari picker. Preview page (`BorrowReceiptPreviewPage`) sudah menyediakan render via `borrowCardPreview(id)`, Cetak (`print.borrowCard(id)`), Simpan PDF (`print.borrowCardPdf(id)`), dan Kembali (`navigate(-1)`). Label `BORROW_CARD_PRINT` dihapus PRINTING/PRINT_SENT/PRINT_ERROR; SUBTITLE → "melihat pratinjau kartunya".
- **Dihapus (4):** `electron/main/menu.ts`, `electron/preload/menu.preload.ts`, `src/components/layout/MenuNavigationListener.tsx`; wiring `menuAPI` dari `electron/preload/index.ts` (import+spread), blok `menu` dari `src/renderer/env.d.ts`, mount listener dari `src/components/layout/AppLayout.tsx`.
- **REVISI 3 (source):** `src/components/layout/Sidebar.tsx` — hapus 2 menuItems (`/backup` & `/restore`, ikon `DatabaseBackup`) + hapus import `DatabaseBackup`; item Sidebar lain tidak berubah.
- **TIDAK diubah:** schema/migration (prisma migrate diff = "This is an empty migration."), engine Backup/Restore/Print (printing:borrowCard **reused** di preview page), PrintService, DTO, `src/pages/BorrowReceiptPreviewPage.tsx`, `src/routes/index.tsx` (route `/borrowings/print-card` sudah ada), `tailwind.config.js` (content globs `./src/components/**` sudah mencakup folder baru), **frame BrowserWindow** (`frame:false`/`titleBarStyle` TIDAK dipakai — native Windows title bar dengan minimize/maximize/close dipertahankan).
- **Keputusan:** "Cetak Kartu Peminjaman" = halaman pemilih kartu **AKTIF** (fetch-all loop 100 filter status ACTIVE di renderer; TIDAK mengubah kontrak `borrowings.findMany`); Exit memakai `window.electronAPI.window.close()` yang sudah ada (minimize/maximize/close di TopBar) — `window-all-closed` memanggil `app.quit()` di Windows.
- **Validation PASS:** lint; build (main **2,078.06 kB identik** · preload **12.30 kB identik** · renderer **1,246.34 kB** −1.86 [3 window-control buttons + ikon Minus/Square/X dihapus dari TopBar]); smoke `file_menu_smoke` **58/58** (FILE_MENU_ITEMS 4 item + urutan + label==LABELS.FILE_MENU + route==ROUTES + satu item exit; native menu DIHAPUS: menu.ts tidak ada + index.ts `Menu.setApplicationMenu(null)` + 0 referensi installApplicationMenu/buildFromTemplate di electron/; source statis: BorrowCardPrintPage TANPA `print.borrowCard(` + pakai `receiptPreviewPath`/`ACTION_PREVIEW`, BorrowReceiptPreviewPage punya 3 API print, TopBar mount FileMenuDropdown + **TANPA window controls** [`window.electronAPI.window.minimize/maximize/close` + `title='Minimize/Maximize/Close'`], Exit via FileMenuDropdown → `window.electronAPI.window.close()`, BrowserWindow tanpa frame:false/titleBarStyle, menu.preload.ts/MenuNavigationListener dihapus, preload/env/AppLayout tanpa menuAPI/menu, **Sidebar TANPA '/backup'/'/restore'/DatabaseBackup** + File dropdown tetap memuat Backup/Restore); grep bundle main `setApplicationMenu(null)`=1/`installApplicationMenu`=0/`buildFromTemplate`=0/`menu:navigate`=0, preload `menuAPI`=0; `prisma migrate diff` = empty. Hasil repackage `dist/` diverifikasi (package:win PASS) — renderer bundle dalam asar byte-identik `out/` (SHA-256 `004C4A7F…`) memuat TopBar tanpa window controls.
- **Laporan:** WORK_ORDER_FILE_MENU_IMPLEMENTATION.md. Status: **DONE - READY review PO** (belum commit, tidak membuka WO berikutnya).

### Pelajaran (retain)
- **Native application menu = SATU baris `Menu.setApplicationMenu(null)`.** Setelah REVISI 2, tidak ada template/`buildFromTemplate`/`installApplicationMenu` sama sekali di `electron/` — menu bar native tidak tampil (Windows/Linux); window tetap ber-frame sehingga title bar Windows (minimize/maximize/close) dipertahankan. Bila fungsi default (copy/paste/zoom/devtools) dibutuhkan di masa depan, tambah shortcut/keyboard handler renderer — jangan hidupkan kembali native menu.
- **Pindah item navigasi = HAPUS dari lokasi lama, JANGAN duplikasi.** REVISI 3 menghapus Backup/Restore dari `Sidebar.tsx` (menuItems + import `DatabaseBackup`) karena File dropdown sudah memuatnya; route/halaman/handler yang sama di-reuse apa adanya. Lokasi akses baru TIDAK menambah channel/logic — satu-satunya perubahan adalah ketersediaan di UI. Smoke membuktikan ketiadaan (`Sidebar.tsx` tanpa `'/backup'`/`'/restore'`/`DatabaseBackup`) DAN kehadiran di lokasi baru (FILE_MENU_ITEMS memuat backup/restore) sekaligus.
- **Window controls = urusan native title bar, bukan TopBar (REVISI 4).** Tiga tombol Minimize/Maximize/Close di TopBar dihapus karena menduplikasi native Windows title bar (window tetap ber-frame, `frame:false`/`titleBarStyle` tidak dipakai). Exit tetap ada di File dropdown (`window.electronAPI.window.close()` di `FileMenuDropdown.handleSelect`); preload `window.*` (minimize/maximize/close) dibiarkan utuh — API dipakai Exit dan dipakai native title bar via OS, bukan dari renderer. Bukti smoke: TopBar tanpa `window.electronAPI.window.minimize/maximize/close()` + tanpa `title='Minimize/Maximize/Close'`, Exit tetap di FileMenuDropdown.
- **Node v22 membuat Module._load read-only** (getter-only) — stub modul Node di smoke WAJIB Module.prototype.require (monkey-patch), bukan Module._load. (Berlaku untuk smoke lain; smoke file-menu pasca REVISI 2 TIDAK lagi men-stub 'electron' — compile list tanpa menu.ts.)
- **Static import mengeksekusi require SEBELUM hook** — smoke yang men-stub modul Node harus `require()` DINAMIS setelah hook terpasang; gunakan import type hanya untuk shape.
- **Config menu = modul MURNI terpisah dari komponen.** `FILE_MENU_ITEMS` (id/label/icon-string-key/route|exit) di-import smoke headless tanpa React/lucide; komponen hanya memetakan icon string → lucide. Ini pola config-leaf yang sama dengan `BORROW_STATUS`/`MEMBER_TYPES`.
- **Navigasi menu renderer = SATU router**: dropdown memakai `useNavigate(route)` langsung — tanpa channel IPC (`menu:navigate`), tanpa preload `menu`, tanpa listener; URL & back/forward tetap konsisten. Native menu File dibuang total karena aksinya kini dari dropdown renderer.
- **Exit via `window.electronAPI.window.close()`** (reuse TopBar) valid karena `window-all-closed` → `app.quit()` di non-darwin — tanpa perlu role quit baru.
- **Preview-first = pisahkan "pilih transaksi" dari "cetak".** Picker hanya menavigasi ke `receiptPreviewPath(id)`; seluruh print/PDF/back berada di `BorrowReceiptPreviewPage` (satu-satunya tempat `print.borrowCard*` dipanggil) — bukti via grep bundle renderer `receipt-preview`=3 dan source picker TANPA `print.borrowCard(`.
- **Tailwind content globs sudah mencakup `./src/components/**`** — file baru di `src/components/layout/` otomatis ter-scan; class `z-[70]` ter-generate sebagai selector escaped `z-\[70\]` di CSS build (grep pakai pola escaped, bukan `z-70`).
- **Smoke compile pola murni:** `npx tsc --module commonjs --moduleResolution node --target es2022 --esModuleInterop --skipLibCheck --rootDir . --outDir <tmp>\out file_menu_smoke/smoke.ts src/components/layout/file-menu-items.ts` lalu `node <tmp>\out\file_menu_smoke\smoke.js <repo-root>` (tanpa DB/Electron; pasca REVISI 2 TIDAK ada 'electron' di compile list — menu.ts sudah dihapus, jadi tidak perlu stub; repo root diteruskan argv[2] untuk assertion source statis via fs).

---

## PRINT-UAT-A6-MEDIA-1 (Debug Origin Cetak): MODE U — kartu 110×60 di media NATIVE A6 105×148 (COMPLETE - READY review PO)

### Ringkasan
- **WO:** mode diagnostik **MODE U (UAT A6)** — mencetak kartu debug 110×60 pada **media NATIVE A6 (105×148 mm)** (driver EPSON L3110 tidak selalu menyediakan custom paper 110×60). Kartu **diputar 90°** → footprint 60×110 agar muat pada A6 **TANPA scaling/pemotongan** (lebar kartu 110mm > lebar A6 105mm). Margin pemusatan `(105−60)/2 = 22.5mm` kiri/kanan + `(148−110)/2 = 19mm` atas/bawah (A6 = media NATIVE, bukan offset aplikasi). Rotasi HANYA penempatan di mode ini — desain fisik kartu TETAP 110×60. `margins: { marginType: 'none' }`, TANPA `PRINT_OFFSET_PT`, TANPA custom left, TANPA transform/scale (selain rotasi konten). READ ONLY + DEBUG; printer TIDAK terhubung → PHYSICAL UAT = NOT TESTED.
- **File diubah (8):** `src/main/services/print-debug.service.ts` (+`generateUatA6DebugHtml()` — @page 105mm 148mm margin 0, `.viewport` 105×148, `.sheet` 110×60 `transform: translate(-50%,-50%) rotate(90deg)`, markup `buildDebugPatternMarkup(110, 60)` byte-identik MODE A/C), `electron/main/services/print.service.ts` (+`getDebugUatA6PreviewHtml()`, +`printDebugGridUatA6()` margins none + pageSize A6 105000×148000), `electron/ipc/print.ipc.ts` (+`printing:debugUatA6Preview`, +`printing:debugUatA6`), `electron/preload/print.preload.ts` (+`debugUatA6Preview`, +`debugUatA6`), `src/renderer/env.d.ts`, `src/pages/PrintOriginDebugPage.tsx` (kartu violet "UAT Media A6 (105×148)" + tombol "Cetak UAT A6 (U)" + preview iframe 105×148 + hint), `src/utils/labels.ts` (+`PRINT_DEBUG.UAT_A6_*`/`HINT_UAT_A6`), `print_debug_origin_smoke/smoke.ts` (+section MODE U 14 assertion).
- **TIDAK diubah:** `printBorrowCard` production, `PRINT_OFFSET_PT`, `printHtml`, mode A/B/C/R/S, `renderPdf`, template kartu, `borrow-card.service.ts`, driver, schema/migration (WO diagnostik).
- **Validation PASS:** lint; build (main **2,080.35 kB** · preload **12.46 kB** · renderer **1,250.95 kB** `index-BkDqhzzi.js`); smoke murni **71/71** (21 + 36 MODE S + 14 MODE U); smoke Electron **40 report PASS** (6 mode: PURE/SIZE/PROD/ROT/SIZE86/**UAT_A6** + eksperimen valid + preview HTML 3685 chars == `generateUatA6DebugHtml`); `UAT_OPTS={"margins":{"marginType":"none"},"pageSize":{"width":105000,"height":148000}}` — TANPA offset; eksperimen valid: UAT_A6 pageSize A6 (beda semua mode) + TANPA custom left (sama PURE/SIZE/ROT/S86, beda PROD) + kartu tetap 110×60; grep bundle main `printing:debugUatA6Preview`=1/`printing:debugUatA6`=2, preload `debugUatA6`=4, renderer `debugUatA6`=2.
- **Laporan:** `WORK_ORDER_PRINT_UAT_A6_MEDIA_1.md`. Status: **DONE - READY review PO** (UAT printer di perangkat PO, tidak commit/push).

### UAT (langkah PO di mesin dengan printer kartu / kertas A6)
1. Sidebar → **Debug Cetak** → kartu violet **"UAT Media A6 (105×148)"**.
2. Klik **"Cetak UAT A6 (U)"** → dialog printer → pilih printer kartu yang sama; pilih media **A6 / 105×148** pada driver bila diminta.
3. **Kartu 110×60 (diputar 90°) tercetak utuh di dalam media A6** → penempatan A6 valid; PO dapat mencetak kartu 110×60 di kertas A6 yang tersedia — rotasi HANYA penempatan, kartu produksi tetap 110×60. Terpotong/scaling → A6 bukan media cocok; dokumentasikan. Konfirmasi manual PO direkomendasikan (smoke membuktikan opsi/pageSize/HTML, bukan hasil fisik).

### Pelajaran (retain)
- **Eksperimen media NATIVE vs custom**: MODE U memakai A6 (native 105×148) sebagai pembanding terhadap MODE S (custom 110×86). Semua mode TANPA offset (`marginType: 'none'`) sehingga gejala pergeseran kiri pada hasil fisik dapat diisolasi ke driver, bukan pipeline aplikasi.
- **Rotasi penempatan ≠ scaling**: kartu tetap 110×60 (`buildDebugPatternMarkup(110, 60)` byte-identik MODE A/C), hanya `.sheet` yang di-rotate 90° dalam viewport A6 — bukti smoke: markup identik + sheet 110×60 tanpa scaling.
- **Margin pemusatan 22.5/19 = geometri pemusatan, bukan offset** — dihitung dari selisih dimensi media vs footprint (`(105−60)/2`, `(148−110)/2`); mode TETAP `marginType: 'none'`.
- Smoke compile & run mengikuti pola WO-2/preview: `--module node16 --moduleResolution node16` (bwip-js transitif via print.service) + outDir DI DALAM repo (Electron abaikan NODE_PATH) + `electron main.cjs <absOutDir>`; hapus folder `out/` hasil compile setelah run.

---

## LABEL REDESIGN v2 (Header Logo + Author + Footer Navy) (COMPLETE - READY review PO)

### Ringkasan
- **WO:** redesain visual label buku (3 kolom × 4 baris, A4, 66×71.25mm/label) — header **logo+identitas** (logo data URI / fallback monogram, libraryName, schoolName) → barcode → inventory → divider → baris **judul+penulis** (ikon buku navy) → **footer navy** (ikon pin + rak). Layout `.label` `space-evenly` → `flex-start`; `.label-book { flex:1 }`; `.label-footer { margin-top:auto }`.
- **4 file dimodifikasi:** `src/main/services/label.service.ts` (template v2 — `BOOK_ICON_SVG`/`PIN_ICON_SVG` self-contained, `labelLogoHtml` fallback `generateLogoMonogramSvg` reuse dari borrow-card.service, `labelHeaderHtml`, `label-author` opsional, `label-footer` + `print-color-adjust: exact`), `electron/main/services/print.service.ts` (+`enrichLabelData()` private async — settings → `resolveAssetPath` (SSOT pembaca logoPath, RFC §12) → `readFileAsDataUri` → `logo: string|''`; `getLabelPreviewHtml`/`printBookLabels` kini async + enrich; `libraryName: data.libraryName ?? settings.libraryName`, `schoolName` dari Settings), `src/pages/LabelPreviewPage.tsx` (+`author` per item dari `book.authors`), `src/shared/dto/print.ts` (aditif opsional: `BookLabelItemData.author?`, `BookLabelData.schoolName?`/`logo?`).
- **TIDAK diubah:** schema/migration (`prisma migrate diff` = "This is an empty migration."), IPC/preload/env.d.ts/bootstrap (channel `printing:bookLabels` reused), **`LABEL_PRINT_CONFIG` byte-identical** terhadap HEAD (ukuran label/kapasitas 12/barcode 37mm terjaga), BorrowCardService/template kartu, DTO non-breaking.
- **Harness baru (untracked):** `label_visual_smoke/main.cjs` (struktur HTML + PDF MediaBox), `label_visual_smoke/geometry.cjs` (bounding-box render nyata), `label_visual_smoke/capture.cjs`.
- **Validation PASS:** lint; build (main **2,072.69 kB** · preload **11.64 kB** · renderer **1,234.38 kB** `index-ByC-EywR.js`); smoke render nyata `main.cjs` **12/12** (1 halaman 12 label, logo kosong → monogram tanpa `data:image`, header libraryName+schoolName, author skip 11/12, footer 12×, tanpa undefined/NaN, barcode+inventory, `@page A4`, `.label` 66×71.25mm, PDF MediaBox A4 594.960×841.920pt = 209.889×297.011mm, `PDF_BYTES=188325`); geometry `geometry.cjs` **8/8** (12 label grid A4 793.7×1122.5px, 66.00×71.25mm, tanpa overlap, urutan zona header→barcode→inventory→divider→book→footer, footer di dalam label, barcode 37.00mm, konten tidak terpotong); regression `borrow_card_print_fix_smoke/main.cjs` **17/17** (printBorrowCard pageSize A6 105000×148000, margins none, landscape false, scaleFactor 1, silent false; **label TANPA pageSize & TANPA resolveA6DeviceName** — scope kartu tidak bocor; preview 12060 chars; PDF A6 298.080×420.000pt).
- **Laporan:** `WORK_ORDER_LABEL_REDESIGN_IMPLEMENTATION.md`. Status: **DONE - READY review PO** (belum commit/push, menunggu instruksi).

### Pelajaran (retain)
- **Template label kini pakai logo & identitas dari MAIN via enrich** — renderer cukup mengirim `items`/`author`/`bookTitle`; `resolveAssetPath` tetap SATU-SATUNYA pembaca logoPath (RFC §12) → logo tidak terbaca = string kosong → fallback monogram (tidak pernah ruang kosong). Pola identik `buildBorrowCardHtml`.
- **Reuse monogram, bukan duplikasi** — `generateLogoMonogramSvg(schoolName, libraryName)` diekspor dari `borrow-card.service.ts` dan dipakai label; jangan buat SVG monogram kedua.
- **Perubahan template = kontrak aditif opsional** (`author?`/`schoolName?`/`logo?`) — smoke regression lama (dto lama tanpa field baru) tetap kompilasi & hijau; buktikan dengan menjalankan smoke, bukan hanya tsc.
- **Verifikasi visual = render nyata + geometry, bukan string-match saja** — `geometry.cjs` mengukur bounding-box tiap zona (urutan vertikal, footer di dalam label, barcode 37mm, tanpa overlap); capture PNG tidak bisa diverifikasi model (bukan saluran visual) → jadikan geometry assert programatik.
- **Compile chain regression print service** (label + borrow-card + barcode + asset + setting): `npx tsc --module node16 --moduleResolution node16 ...` list file lengkap (print.service, errorHandler, setting.repository, database, setting.service, borrow.repository, base.*, label.service, barcode.service, borrow-card.service, asset-resolver, logo-config, logo-resize, fs-utils, config leaves) → outDir DI DALAM repo (Electron abaikan NODE_PATH; bwip-js/sharp resolve dari root node_modules) → `electron main.cjs <outDir> <pdf>`; hapus `out/` hasil compile setelah run.
- **Regression smoke mengunci scope negatif**: label path memanggil `webContents.print` TANPA `pageSize` (A4 default) dan TANPA `resolveA6DeviceName` — membuktikan parameter kartu (A6 105×148) tidak bocor ke label; `LABEL_PRINT_CONFIG` byte-identical dibuktikan via `git diff` tanpa `+/-` pada blok config (bukan string-compare yang kena line-ending/coercion PowerShell).

---

## WO Inventory Prefix Alignment: Satu Counter, Pasangan INV + Prefix (COMPLETE — READY review PO, commit terpisah)

### Ringkasan
- Melengkapi 2 WO sebelumnya (`dd1c237` barcodeFormat, `56be610` inventoryPrefix): **`inventoryNumber` SELALU `INV-XXXXXX`** (identitas stabil, healing/reconciliation pakai needle TETAP `'INV-'`), **`barcode` = `<Setting.inventoryPrefix>-XXXXXX`** — keduanya memakai **SATU counter urutan yang sama**.
- **6 file source + smoke diubah, 2 file baru:** `src/main/services/inventory-allocator.ts` (return `Array<{inventoryNumber,barcode}>` dari satu counter; healing needle `'INV-'`), `electron/main/services/inventory-allocator.ts` (pasangan sama, tanpa healing), caller `electron/main/services/book-copy.service.ts` + `src/main/services/book-import.service.ts` (destructure pasangan), `src/main/services/database-reconciliation.service.ts` (needle `'INV-'` tetap, prefix setting hanya kosmetik), `scripts/migrate-inventory-number.ts` (**baru** — migrasi data sekali-jalan: `inventoryNumber` legacy `BC-…` → `INV-<seq>`, `barcode` TIDAK diubah, idempoten, lapor collision/anomaly), `inventory_prefix_smoke/smoke.ts` (ditulis ulang ke kontrak pasangan, 36 assertion), `WORK_ORDER_INVENTORY_PREFIX_ALIGNMENT.md` (**baru**).
- **Eksekusi migrasi dev DB (2026-08-10):** backup `backup/inventory-prefix-20260810/aplibrary.db`; **totalNonInv 11 · migrated 11 · orphan 0 · collision 0**; semua `BC-…` → `INV-<seq>`, `barcode` dipertahankan; idempoten; verifikasi pasca tidak ada non-`INV-` tersisa.
- **TIDAK diubah:** schema/migration (`prisma migrate diff` = "This is an empty migration."), `borrow-card.service.ts`/`print.service.ts` (diff kosong), IPC/preload/env/bootstrap, UI.
- **Validation PASS:** lint; build (main **2,075.35 kB** · preload 11.64 kB · renderer 1,240.29 kB); smoke `inventory_prefix_smoke` **36/36**; regression fresh DB: `wo21_import_b1b2` 48/48 · `it1_borrow_return` 34/34 · `it_borrow_eligibility` 7/7 · `wo14_e2` 36/36 · `settings_db_reset` 58/58 · `barcode_format` 23/23 (murni).
- **Temuan terpisah (TECH DEBT):** `borrow_card_uat_smoke` **17 PASS / 14 FAIL** — dibuktikan **pre-existing** (identik pada tree bersih `56be610` sebelum WO ini; smoke terakhir di-update era kartu 110×60 `09c7910`/`1a90725`, template sudah A6 `8592dd0`). Bukan regresi WO ini; **DILARANG diperbaiki di sini** — tindak lanjut: WO update smoke ke markup A6.
- **Laporan:** `WORK_ORDER_INVENTORY_PREFIX_ALIGNMENT.md`. Status: **DONE — READY review PO** (commit terpisah dari 2 commit WO sebelumnya).

### Pelajaran (retain)
- **Satu counter, dua kolom — pasangan 1:1** dalam satu transaksi; nomor urut tidak di-reset saat prefix berubah. Allocator (baru & legacy) mengembalikan `{inventoryNumber:'INV-…', barcode:'<prefix>-…'}`; caller destructure pasangan, bukan string saja.
- **`barcode` = kolom yang memakai prefix konfigurable; `inventoryNumber` tetap `'INV-'`** agar identitas stabil. Healing & reconciliation memakai needle TETAP `'INV-'` pada kolom `inventoryNumber` — nilai legacy `BC-…` TIDAK memengaruhi urutan; prefix setting hanya nilai kosmetik record sequence.
- **Migrasi data sekali-jalan terpisah dari kode**: `scripts/migrate-inventory-number.ts` menyeragamkan `inventoryNumber` legacy → `INV-<seq>` TANPA menyentuh `barcode` (label/QR lama tetap valid); idempoten + lapor collision/anomaly; jalankan hanya dengan stop aplikasi + backup + preflight (prosedur backfill).
- **Verifikasi regresi vs stale smoke**: ketika smoke lain gagal, buktikan pre-existing dengan `git stash push -u` → run smoke pada tree bersih (HEAD sebelum WO) → bandingkan hasil (identik 17/14) → `git stash pop`. JANGAN langsung "perbaiki" smoke di WO yang scope-nya berbeda — catat sebagai tech debt di laporan.

---

## MIGRATION BOOTSTRAP (Installer Audit Fix #3): skema DB dibuat otomatis di instalasi bersih (COMPLETE - READY review PO)

### Ringkasan
- **Fix #3 (Installer Audit):** pada instalasi bersih packaged, DB userData baru kosong dan aplikasi crash di `initDatabase`/reconciliation (tabel `InventorySequence` belum ada). Bootstrap migration ala `prisma migrate deploy` dijalankan otomatis saat startup packaged.
- **File baru (2):** `src/main/infrastructure/migrations/sql-split.ts` (`splitSqlStatements` murni — state machine string tunggal/ganda [escape `''`/`""`], line comment `--`, block comment `/* */`, statement diakhiri `;`), `src/main/infrastructure/migrations/migration-bootstrap.ts` (`MIGRATIONS_TABLE_DDL` persis kontrak Prisma 5.22 8 kolom, `computeMigrationChecksum` sha256, `loadMigrationFiles` urut lexicographic + abaikan `migration_lock.toml`, `applyPendingMigrations` transaction-per-migration + INSERT record `_prisma_migrations` [checksum = sha256 file, startedAt/finishedAt epoch ms, applied_steps_count = jumlah statement], `bootstrapMigrations` fail-fast bila tidak ada migration).
- **Dimodifikasi (1):** `electron/main/index.ts` — dalam `app.whenReady()` blok `app.isPackaged`: `bootstrapMigrations(path.join(process.resourcesPath, 'migrations'))` **sebelum** `initDatabase()` dan reconciliation; log `[Migrations] bootstrap: N applied, M skipped`.
- **TIDAK diubah:** schema, migration (bootstrap memakai 7 file yang ada), `electron-builder.yml` (`extraResources` **sudah** memuat `prisma/migrations/ → resources/migrations/`), engine (`PRISMA_QUERY_ENGINE_LIBRARY` Fix #1 module-scope sudah diarahkan), DB dev, Backup/Restore, UI.
- **Interchangeability:** record `_prisma_migrations` hasil bootstrap identik kontrak Prisma CLI sehingga DB hasil bootstrap == DB hasil `migrate deploy`; `prisma migrate status` melaporkan *up to date* (dibuktikan smoke), backup/restore `SchemaVersionReader` bekerja tanpa perubahan.
- **Validation PASS:** smoke `migration_bootstrap_smoke` **83/83** (splitter 11, checksum 4, loader 6, statement counts persis plan [adr002=47/wo13=3/r1=2/f2a=14/auth1=3/auth7=4/wo_print=1], DDL 4, bootstrap segar 10 [7 applied, 19 tabel model ada, checksum DB==sha256 file, applied_steps_count benar, insert+read], idempotensi 3, rollback transaction-per-migration 5 [statement gagal → rollback migrasi itu saja, migrasi sebelumnya tetap tercatat], fail-fast 1, `prisma migrate status` 2 [exit 0 + "up to date"]); lint PASS; build PASS (main 2,397.24 kB); bundle grep `bootstrapMigrations`×2/`[Migrations] bootstrap`×1. DB dev tidak disentuh (fresh DB temp).
- **Laporan:** `WORK_ORDER_MIGRATION_BOOTSTRAP.md`. Status: **DONE - READY review PO** (tidak membuka WO berikutnya).

### Pelajaran (retain)
- **Source tunggal migration dev & production** = file yang sama (`prisma/migrations/`); `extraResources` sudah menyimpan ke `resources/migrations/`. `migration_lock.toml` di-abaikan loader (bukan dir ber-`migration.sql`).
- **Format `_prisma_migrations` wajib persis Prisma** — `prisma migrate status` adalah penguji integritas terbaik (checksum/format/counts menyimpang = ditolak CLI). `applied_steps_count` dibaca **BigInt** dari SQLite: jangan `===` angka tanpa `Number()`, jangan `JSON.stringify` BigInt (crash serialize).
- **Transaction-per-migration**, bukan satu transaksi raksasa — kegagalan satu statement me-rollback hanya migrasi itu; migrasi lain tetap tercatat (perilaku sama `prisma migrate deploy`).
- **Splitting SQL harus state machine** (bukan `split(';')`) — semicolon di dalam literal/komentar/identifier ber-quote; dukung escape `''`/`""`.
