...
const MEMBER_TYPES = ['student', 'teacher', 'staff'] as const
...
const EDIT_MOCK: FormData = {
  memberNumber: 'AGT-000001',
  ...
  postalCode: '12430',
  notes: 'Anggota baru tahun ajaran 2025/2026.'
}

export default function MemberForm({ mode = 'create', initialData }: MemberFormProps) {
  const editInitial = mode === 'edit' ? (initialData ?? EDIT_MOCK) : {}
  ...
  const [memberType, setMemberType] = useState(editInitial.memberType ?? '')
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!validate()) return
    if (isEditMode) {
      alert('Fitur Simpan Perubahan belum tersedia karena modul Anggota masih dalam pengembangan.')
    } else {
      alert('Fitur Simpan Anggota belum tersedia karena modul Anggota masih dalam pengembangan.')
    }
  }
