...
import type { OverdueCategory, OverdueReportDTO } from '../../shared/dto/report'
...
const STATUS_BADGE: Record<OverdueCategory, { label: string; className: string }> = {
  ACTIVE: { label: LABELS.REPORT.STATUS_STILL_LATE, className: 'bg-rose-100 text-rose-700' },
  RETURNED: { label: LABELS.REPORT.STATUS_RETURNED_LATE, className: 'bg-amber-100 text-amber-700' },
}
...
export default function OverdueReportPage() {
  ...
  const [report, setReport] = useState<OverdueReportDTO | null>(null)
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">{LABELS.REPORT.OVERDUES}</h1>
        <p className="text-sm text-slate-500 mt-1">{LABELS.REPORT.OVERDUES_DESC}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <StatCard
          ...
          label={LABELS.REPORT.TOTAL_OVERDUE}
          value={loading ? '...' : report?.pagination.total ?? '—'}
          color="text-rose-600 bg-rose-50"
        />
        <StatCard
          icon={<Clock size={20} />}
          label={LABELS.REPORT.STILL_LATE}
          value={loading ? '...' : summary?.active ?? '—'}
          color="text-orange-600 bg-orange-50"
        />
        <StatCard
          icon={<Undo2 size={20} />}
          label={LABELS.REPORT.RETURNED_LATE}
          value={loading ? '...' : summary?.returned ?? '—'}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-left text-slate-500">
                    <th className="pb-3 font-medium">{LABELS.REPORT.COL_BORROW_DATE}</th>
                    ...
                    <th className="pb-3 font-medium">{LABELS.REPORT.COL_LATE_DAYS}</th>
                  ...
                  {report.rows.map((row, idx) => (
                    <tr key={`${row.borrowNumber}-${idx}`} className="border-b border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="py-3 text-slate-700 font-medium whitespace-nowrap">
                        {row.lateDays} {LABELS.REPORT.DAYS}
                      </td>
                      <td className="py-3">
                        <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_BADGE[row.category].className}`}>
                          {STATUS_BADGE[row.category].label}
                        </span>
                      </td>
                    </tr>
                  ))}
          ...
          {report && report.pagination.total > 0 && (
            <div className="flex items-center justify-between pt-4 border-t border-slate-200 mt-4">
              <p className="text-sm text-slate-500">
                Total {report.pagination.total} terlambat
              </p>
