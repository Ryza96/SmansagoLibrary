...
import type { BorrowReportStatus, BorrowingReportDTO } from '../../shared/dto/report'
...
const STATUS_BADGE: Record<BorrowReportStatus, { label: string; className: string }> = {
  ACTIVE: { label: LABELS.REPORT.STATUS_ACTIVE, className: 'bg-emerald-100 text-emerald-700' },
  COMPLETED: { label: LABELS.REPORT.STATUS_COMPLETED, className: 'bg-slate-100 text-slate-600' },
  OVERDUE: { label: LABELS.REPORT.STATUS_OVERDUE, className: 'bg-rose-100 text-rose-700' },
}
...
const STATUS_OPTIONS: { value: '' | BorrowReportStatus; label: string }[] = [
  { value: '', label: LABELS.REPORT.STATUS_ALL },
  { value: 'ACTIVE', label: LABELS.REPORT.STATUS_ACTIVE },
  { value: 'COMPLETED', label: LABELS.REPORT.STATUS_COMPLETED },
  { value: 'OVERDUE', label: LABELS.REPORT.STATUS_OVERDUE },
]

export default function BorrowingReportPage() {
  const [from, setFrom] = useState('')
  const [to, setTo] = useState('')
  const [status, setStatus] = useState<'' | BorrowReportStatus>('')
  const [search, setSearch] = useState('')
  const [page, setPage] = useState(1)
  const [loading, setLoading] = useState(true)
  const [report, setReport] = useState<BorrowingReportDTO | null>(null)
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">{LABELS.REPORT.BORROWINGS}</h1>
        <p className="text-sm text-slate-500 mt-1">{LABELS.REPORT.BORROWINGS_DESC}</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <label className="block">
            <span className="text-xs font-medium text-slate-500">{LABELS.REPORT.STATUS}</span>
            <select
              value={status}
              onChange={(e) => {
                setStatus(e.target.value as '' | BorrowReportStatus)
                setPage(1)
              }}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          label={LABELS.REPORT.TOTAL_TRANSACTIONS}
          value={loading ? '...' : summary?.total ?? '—'}
          color="text-blue-600 bg-blue-50"
        />
        <StatCard
          icon={<BookMarked size={20} />}
          label={LABELS.REPORT.ACTIVE}
          value={loading ? '...' : summary?.active ?? '—'}
          color="text-emerald-600 bg-emerald-50"
        />
        <StatCard
          icon={<BookUp size={20} />}
          label={LABELS.REPORT.COMPLETED}
          value={loading ? '...' : summary?.completed ?? '—'}
          color="text-slate-600 bg-slate-100"
        />
        <StatCard
          icon={<TriangleAlert size={20} />}
          label={LABELS.REPORT.OVERDUE}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-left text-slate-500">
                    <th className="pb-3 font-medium">{LABELS.REPORT.COL_DATE}</th>
                    ...
                    <th className="pb-3 font-medium">{LABELS.REPORT.COL_DUE}</th>
                ...
                <tbody>
                  {report.rows.map((row, idx) => (
                    <tr key={`${row.borrowNumber}-${idx}`} className="border-b border-slate-100 hover:bg-slate-50">
                      <td className="py-3 text-slate-600 whitespace-nowrap">{formatReportDate(row.borrowDate)}</td>
                      ...
                      <td className="py-3 text-slate-600 whitespace-nowrap">{formatReportDate(row.dueDate)}</td>
                      <td className="py-3">
          ...
          {report && report.pagination.total > 0 && (
            <div className="flex items-center justify-between pt-4 border-t border-slate-200 mt-4">
              <p className="text-sm text-slate-500">
                Total {report.pagination.total} transaksi
              </p>
